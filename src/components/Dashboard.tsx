...
export function Dashboard({ runs }: { runs: Run[] }) {
  ...
  return <div className="dashboard">
    ...
    <div className="chart-grid">
      ...
      <Chart title="Card signals" subtitle="Lift versus your overall win rate · minimum 3 runs" wide>{cards.length ? <div className="signal-list">{cards.map((card) => <div key={card.name}><span>{card.name}<small>{card.runs} runs</small></span><strong className={card.lift >= 0 ? 'positive' : 'negative'}>{card.lift >= 0 ? '+' : ''}{card.lift.toFixed(1)} pp</strong></div>)}</div> : <NoData>Repeated card picks will reveal which ones outperform your baseline.</NoData>}</Chart>
    </div>
  </div>
}
