...
export const isRun = (value: unknown): value is Run => {
  ...
  return typeof run.id === 'string' && typeof run.date === 'string' && CHARACTERS.includes(run.character) && Number.isInteger(run.ascension) && run.ascension >= 0 && OUTCOMES.includes(run.outcome) && Number.isInteger(run.floor) && run.floor >= 0 && Array.isArray(run.cards) && run.cards.every(isPickup) && isOptionalStringArray(run.cardsEverOwned) && isOptionalStringArray(run.cardsRemovedDuringRun) && isOptionalStringArray(run.cardEffects) && (run.cardChanges === undefined || (Array.isArray(run.cardChanges) && run.cardChanges.every(isCardChange))) && Array.isArray(run.relics) && run.relics.every(isPickup) && (run.relicChanges === undefined || (Array.isArray(run.relicChanges) && run.relicChanges.every(isRelicChange))) && isOptionalStringArray(run.potions) && isOptionalStringArray(run.finalPotions) && (run.potionChanges === undefined || (Array.isArray(run.potionChanges) && run.potionChanges.every(isPotionChange)))
}
...
export function mergeRuns(existing: Run[], incoming: Run[]) {
  ...
  return [
    ...existing.map((run) => {
      ...
      return relicChanges !== undefined || cardsEverOwned !== undefined || cardsRemovedDuringRun !== undefined || cardChanges !== undefined || finalPotions !== undefined || potionChanges !== undefined
        ? { ...run, ...(relicChanges !== undefined ? { relicChanges } : {}), ...(cardsEverOwned !== undefined ? { cardsEverOwned } : {}), ...(cardsRemovedDuringRun !== undefined ? { cardsRemovedDuringRun } : {}), ...(cardChanges !== undefined ? { cardChanges } : {}), ...(finalPotions !== undefined ? { finalPotions } : {}), ...(potionChanges !== undefined ? { potionChanges } : {}) }
        : run
    }),
